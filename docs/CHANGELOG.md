...
### Fixed
- **Server log said "You are now known as X" for other people's renames.** The engine publishes a nick change twice — the base event with no channel, then one copy per affected channel (`engine/source/ircfiber/irc/connection.d:6885`, `:6894-6899`) — and the frontend routes a channel-less event to `_server` (`messageHandler.ts:182`). `classifyServerLog` called *any* `_server` `NICK` a `self` event on command equality alone, so a channel-mate's rename rendered in the first person in the viewer's own server log. The engine now stamps the existing `self_echo` tag (wire `se`) on our own NICK (`connection.d`, `if (isSelf)` branch), and `serverLogGroups.ts` classifies a `_server` `NICK` as `self` only when `msg.selfEcho` is set, otherwise `skip` — the same treatment the channel-less `QUIT` twin already had. Foreign renames still render in the channel buffers, which get their own per-channel copy. Server-log rows written before this deploy carry no `se`, so past self-renames no longer appear in the server log (channel buffers and scrollback are unaffected).
- **Six gateway entry points addressed a network by client-supplied UUID with no owner check.** `getMessages`, `reconnectNetwork`, `disconnectNetwork`, `joinChannel` and `partChannel` (`site/backend/source/ircfiber/api/rest.d`) now use the fail-closed `findByIdWithUser` → 404 → 403 `not your network` idiom already used by `getOOBEvents`. `GET /api/channels/:network/_server/messages` previously served any logged-in caller another account's server log verbatim, and `POST /api/networks/:id/reconnect` re-homed another account's connection to the caller's owner id (the engine trusts `ControlMessage.userId` verbatim), which would have redirected the victim's live event stream. In `websocket.d`, every network-scoped client command (`msg`, `editmsg`, `join`, `part`, `raw`, `buffer`) is now gated on the session user owning the id (cached `findByUserId`, fail closed), and a WS session restored from Redis is refused unless its user matches the authenticated user — previously the session was adopted on the JWT's `sessionId` alone while the live subscription used the authenticated user id and the backlog replay used the restored session's user id.
